...
export default function PhysicalTraitsPanel({ isDarkMode = false, theme, data }: PhysicalTraitsPanelProps) {
  ...
  // Check if physical traits data is available from the database
  const hasRealData = data?.physical_traits && data.physical_traits.length > 0
  const physicalTraits = hasRealData ? data.physical_traits : []
  ...
  const traits = (data.physical_traits || []).map((trait: any) => ({
    category: trait.trait_name,
    trait: trait.trait_value,
    gene: trait.associated_gene || 'Multiple',
    probability: trait.confidence_score * 100 || 50,
    description: trait.description || 'Genetic predisposition analysis'
  }))
