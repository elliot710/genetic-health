...
export default function DetoxPanel({ data, isDarkMode, theme }: DetoxPanelProps) {
  ...
  // Extract detox data from analysis results
  const detoxData = data?.detox || {
    phase1_capacity: 'Normal',
    phase2_capacity: 'Slow',
    overall_detox_score: 75,
    risk_genes: ['GSTM1', 'GSTT1'],
    recommended_support: ['NAC', 'Glutathione', 'Milk Thistle'],
  ...
  const detoxPhases = [
    {
      phase: 'Phase I',
      name: 'Oxidation',
      capacity: detoxData.phase1_capacity || 'Normal',
    ...
    },
    {
      phase: 'Phase II',
      name: 'Conjugation',
      capacity: detoxData.phase2_capacity || 'Normal',
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className={`${glassBackground} border ${borderColor} rounded-xl p-6`}>
            ...
            <div className={`text-2xl font-bold ${textPrimary} mb-2`}>
              {detoxData.overall_detox_score}/100
            </div>
          ...
          <div className={`${glassBackground} border ${borderColor} rounded-xl p-6`}>
            ...
            <div className={`text-lg font-bold ${getCapacityColor(detoxData.phase1_capacity)} mb-2`}>
              {detoxData.phase1_capacity}
            </div>
            <div className={`text-xs ${textSecondary}`}>
              Oxidation capacity
            </div>
          ...
          <div className={`${glassBackground} border ${borderColor} rounded-xl p-6`}>
            ...
            <div className={`text-lg font-bold ${getCapacityColor(detoxData.phase2_capacity)} mb-2`}>
              {detoxData.phase2_capacity}
            </div>
            <div className={`text-xs ${textSecondary}`}>
      ...
      <div className={`${glassBackground} border ${borderColor} rounded-2xl p-8`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {(detoxData.risk_genes || ['GSTM1', 'GSTT1']).map((gene: string, index: number) => (
            <div key={index} className={`${glassBackground} border ${borderColor} rounded-xl p-6`}>
              ...
              <div className={`px-3 py-1 bg-red-500/20 text-red-400 rounded-full text-xs font-medium inline-block`}>
                High Risk
              </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-3">
              {(detoxData.recommended_support || ['NAC', 'Glutathione', 'Milk Thistle']).map((supplement: string, index: number) => (
